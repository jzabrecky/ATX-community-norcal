...
chla_pheo_final <- chla_pheo_calculations %>% 
  mutate(neg_Pheo_flag = case_when(Pheo_ug_L < 0 ~ "y",
         ...
         Chla_Pheo_flag = case_when(RFU_bdl == 1 ~ "y",
                                    TRUE ~ "n")) %>% 
  select(field_date, site_reach, triplicate, Chla_ug_L, Pheo_ug_L, Chla_Pheo_flag, neg_Pheo_flag)

# save final csv
write.csv(chla_pheo_final, "./data/NT_misc/NT_chlorophylla_unprocessed.csv", )
...
#### PROCESSING CAN MOVE TO ANOTHER SCRIPT
